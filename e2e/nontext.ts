 ...
 *
 * In this repo the FIRST half is the one that matters. Shadow Vault is a form:
 * two passphrase inputs, two textareas, a radio group restyled by
 * `@tailwindcss/forms` (`appearance: none`, so every pixel of it is the
 * author's to answer for), three range sliders whose track is repainted by
 * `src/style.css`, three `<select>`s, a `role="button"` drop zone whose dashed
 * border is its whole identity, and a dozen buttons on three different
 * surfaces (`--vault-bg`, `--vault-surface`, and the always-dark top bar).
 * Almost every one of those identifies itself by a `--vault-border` line or a
 * fill against its card, and none of that is judged by axe or reachable by the
 * text walk. This repo's own history is the proof the class is live here: the
 * unpushed commit before this gate landed fixed the shared header's ghost
 * buttons, whose 1px border measured 1.46:1 — found by exactly this oracle
 * running in a sibling lab. It measures the composite each control is really
 * drawn on, at every driven state, including `:hover` — which persists after a
 * click and is the state a reader occupies the instant after pressing CREATE
 * VAULT.
 *
 * The second half is inert in this repo today — Shadow Vault's stylesheet
 * declares no `content` rule at all; its ▸ / ✓ / ✗ step glyphs and the 👁
 * reveal toggle are real text nodes the arithmetic walk already measures. It
 * runs at every state anyway, so that stays a measurement rather than a
 * reading of the stylesheet, and the first pseudo-element glyph anyone adds is
 ...
 *    That is exactly the shape this lab's inputs have: they are filled with
 *    `--vault-bg`, darker than their card by well under 3:1, so their
 *    `--vault-border` line is the ONLY thing that makes the control visible.
 *
 ...
 * background beneath — because the surfaces behind this lab's controls include
 * the body's `url()` noise layer over `--vault-bg`, the top bar's
 * `color-mix(…, transparent)` edges over its fixed `#0b1512`, and the hero
 * aside's `color-mix(in oklab, …)` wash, and reading a declared value there
...
export async function auditNonText(page: Page, within = 'body *'): Promise<NonTextFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     * worst *stop*, assuming that stop covered the control wherever it sat.
     * That is right only for a gradient whose worst stop spans its element, so
     * each gradient is *sampled* at the control's real location instead: linear
     ...
     * is unmeasurable and paints nothing — the branch this page actually
     * exercises, on `<body>`'s SVG noise texture and the `<select>` arrow
     * `@tailwindcss/forms` draws as a background-image. Shadow Vault declares
     * no gradient of its own today; the sampler stays so the first one added is
     * measured rather than mistaken for a flat fill.
     */
     ...
     *
     * A driven pass here walks both tab panels at once, and the expensive part
     * is the generated-content half: it asks for TWO extra computed styles
     * (`::before` and `::after`) on every element on the page, and
     * `visualizer.ts` renders 512 sibling `<div class="container-cell">`s.
     ...
     * what a page paints. Elsewhere in this fleet that exact bug reported a
     * bright disclosure triangle at 3.95:1 over a near-black canvas, and the
     * tell was that the number came out IDENTICAL in both themes — a ratio
     * that cannot depend on the theme is not measuring the theme. This lab's
     * `html` does carry an explicit `background-color: var(--vault-bg)`, so
     * the crudest failure cannot happen here today, but its `<body>` layers an
     * `url()` noise image over that colour and `@tailwindcss/forms` draws the
     * `<select>` arrow as a background-image — both must composite as LAYERS
     * over the colour, not replace it. It borrows `contrast.ts`'s
     * gradient-aware sampler wholesale rather than approximating it a second
     * time.
     */
    ...
    const CONTROL = [
      ...
      // control?" test below. This lab's footer Source / related-demo links and
      // the modal's VeraCrypt link are plain prose links (no `btn` in any
      // class), so they are not judged here; their ink is measured all the same
      // by the 1.4.3 walk in `contrast.ts`.
      'a[role=button]',
      'a[class*=btn]',
      'a[class*=button]',
    ].join(',');
    ...
    for (const el of Array.from(document.querySelectorAll(CONTROL))) {
      ...
      // the ENCRYPT/DECRYPT row does not jump when the crimson selection
      ...
      const hasOutline =
        outlineW > 0 && cs.outlineStyle !== 'none' && (outlineInk?.a ?? 0) > 0;
      ...
      if (hasOutline) {
        ...
        const oc = outlineInk ?? TRANSPARENT;
